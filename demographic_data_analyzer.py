...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round(len(num_min_workers[df.salary == '>50K'])/len(num_min_workers)*100, 1)

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = (round(df['native-country'][df.salary == '>50K'].value_counts() / df['native-country'].value_counts()*100,1)).idxmax()
    highest_earning_country_percentage = (round(df['native-country'][df.salary == '>50K'].value_counts() / df['native-country'].value_counts()*100,1)).max()
